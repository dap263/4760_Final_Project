...
int PT_GetMachineBuffer1(struct pt *pt, char* buffer)
{
    ...
    PT_timeout = 0;
    // clear input buffer
    memset(buffer, 0, max_chars_GPS);
    
    while(num_char < max_chars_GPS)
    {
        // get the character
...
#define UART1_RX_PIN RPA2 // The RX pin must be one of the Group 3 input pins: RPA2, RPB6, RPA4, RPB13, RPB2
#define UART1_TX_PIN RPB7 // The TX pin must be one of the Group 1 output pins: RPA0, RPB3, RPB4, RPB15, RPB7
...
//   normal_text ;
  ...
  DmaChnSetTxfer(DMA_CHANNEL1, PT_send_buffer+1, (void*)&U2TXREG, max_chars_GPS, 1, 1);
  // signal when done
